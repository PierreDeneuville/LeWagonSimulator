...
daily_challenges_array[0..4].each do |daily_challenge|
  success_probability.each do |proba|
    Exercise.create(position: success_probability.find_index(proba) + 1,
...
daily_challenges_array[5..9].each do |daily_challenge|
  success_probability.each do |proba|
    Exercise.create(position: success_probability.find_index(proba) + 1, success_probability: proba, daily_challenge: daily_challenge)
  end
end
...
success_probability = [25, 25, 20, 25, 20, 16]
